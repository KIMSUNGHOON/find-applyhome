...
동시성 제어(동시 4개, 요청 간 0.1초)가 서버에서 브라우저로 이동한다.
...
브라우저가 `/api/dongs` 를 먼저 부르고, 이어서 동마다 `/api/hos` 를 동시 4개로 부른다.
...
2. 동마다 /api/hos 를 동시 4개로 불러 호 목록과 격자를 받고, 빈 격자를 그린다
3. /api/pblanc 로 공고 정보를 받는다 (실패해도 스캔은 진행)
4. 전체 (동, 호) 목록에 대해 동시 4개로 /api/unit 을 호출한다
   - 워커마다 요청 사이 0.1초 대기
...
`AbortController` 로 중단 버튼을 구현한다. 지금은 `EventSource.close()` 가 그 역할을 한다.
...
- 브라우저에서 **동시 4개 + 요청 간 0.1초** 를 유지한다. 지금 로컬 실행과 같은 수준이다
