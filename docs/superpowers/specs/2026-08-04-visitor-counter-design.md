...
- 카운터와 스캔 캐시는 `UpstashTransport` 의 회로 차단기를 공유한다. 카운터 쪽 Redis 호출이
  실패해 차단기가 열리면 60초 동안 스캔 캐시도 `disabled` 로 떨어진다. 그때도 스캔은 멈추지
  않는다 — 캐시를 건너뛰고 청약홈을 직접 조회하는 기존 degrade 경로로 간다. transport 를
  분리하면 이 결합을 끊을 수 있지만, Redis 설정이 두 벌이 되는 비용이 실제 발생 확률에 비해
  크다고 판단해 하나의 `CACHE` 싱글턴을 그대로 공유한다
- `GET /api/visits` 를 반복해 부르면 `visits:total` 을 임의로 부풀릴 수 있다. `visits:day:`
  는 48시간 뒤 사라지지만 `visits:total` 은 TTL 이 없어 부풀린 값이 영구로 남는다. IP 나
  User-Agent 판별 없이는 싸게 막을 수 없고, 표시용 숫자이므로 받아들인다
